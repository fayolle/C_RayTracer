...
void camera_pos(camera *cam, vec3 lookfrom, vec3 lookat, 
    vec3 vup, float vfov, float aspect, float aperture, float focus_dist) 
{
...
vec3 random_in_unit_disk() {
    ...
    do {
        p = vec3_subtract_vec(
            vec3_multiply_float((vec3) { .x = unitrand(), .y = unitrand(), .z = 0 }, 2.0),
            (vec3) {
            .x = 1, .y = 1, .z = 0
        }
        );
